...
def map_gene_list(gene_list, gene_info):
    """
     geneset 
        ...
        gene_list: list[str] alias/
        geneset: set  list[str] HGNC 
        map_dict: dict, alias  approved name 

    
        mapped_genes: list[str] geneset 
        failed_genes: list[str] geneset 
    """
...
class SCrna():
    ...
    def __getitem__(self, idx):
        ...
        celltype_label = self.celltypes_labels[idx]  # celllabel
        batch_id = self.batch_ids[idx]               # cellbatch id
        return data, gene, T, celltype_label, batch_id
...
class TestSCrna():
    ...
    def __getitem__(self, idx):
        x = self.masked_X[idx].reshape(-1).astype(np.float32)  # masked 
        y = self.label_padded[idx].reshape(-1).astype(np.float32)  # ground truth
        m = self.mask_padded[idx].reshape(-1).astype(bool)  # mask 
        g = self.gene
        return x, g, y, m
...
def build_dataset(dataset, prep, batch_size, pad_zero=True, drop=True, shuffle=True):
    def collate_fn(samples):
        ...
        for data, gene, T, celltype_label, batch_id in samples:
            # 1. /
            ...
            if pad_zero:
                ...
                gene, z_gene, z_sample = prep.compress(gene, z_sample)  #  MindSpore 
