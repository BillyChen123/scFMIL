...
class FinetuneModel(nn.Module):
    ...
    def embedding_infer(self, expr, gene, ST_feat, zero_idx):
        b, l = gene.shape
        gene_emb = self.gene_emb[gene] # gene emb  27856gene b*l
...
if __name__ == '__main__':
    ...
    # ===  optimizer &  moment1/2  optimizer.state ===
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    net = net.to(device)
    optimizer = torch.optim.Adam(net.parameters(), lr=1e-4, weight_decay=1e-5)
    scaler = torch.cuda.amp.GradScaler(
        init_scale=1.0,  #  loss scale
    )

    # =========================================================================
    #  param_name  param tensor 
    param_name_map = {name: param for name, param in net.named_parameters()}
    #  PyTorch optimizer state param  key
    moment_states = {}
    for name, param in param_name_map.items():
        if name in moment_state_dict_1 and name in moment_state_dict_2:
            ...
            step = torch.tensor(0, dtype=torch.float32, device='cpu')  #  CPU
            moment_states[param] = {
                'exp_avg': exp_avg,
